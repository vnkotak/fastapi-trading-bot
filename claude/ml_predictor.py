...
class MLPredictor:
        ...
        try:
            if os.path.exists("models/rf_model.pkl") and os.path.exists("models/scaler.pkl"):
                ...
                if os.path.exists("models/feature_names.pkl"):
                    ...
                    # Default feature names (should match FeatureEngineer output)
    ...
    def _get_default_feature_names(self):
        """
        Get default feature names (should match FeatureEngineer)
        """
        return [
            "rsi", "rsi_change", "rsi_oversold", "rsi_overbought",
            "macd", "macd_signal", "macd_histogram", "macd_bullish",
            "ema20_slope", "ema50_slope", "price_above_ema20", "price_above_ema50",
            "bb_position", "bb_squeeze", "stoch_k", "stoch_d", "stoch_bullish",
            "williams_r", "williams_oversold", "price_volume_corr", "intraday_strength",
            "avg_gap_ratio", "momentum_1d", "momentum_3d", "momentum_5d", "momentum_10d",
            "momentum_consistency", "beta", "relative_strength", "correlation", "outperforming_market",
            "body_size", "upper_shadow", "lower_shadow", "price_in_range", "higher_highs",
            "higher_lows", "bullish_structure", "doji", "hammer", "volume_ratio",
            "volume_trend", "volume_surge", "volume_dry_up", "price_volume_divergence",
            "obv_trend", "atr_ratio", "volatility_rank", "realized_volatility",
            "volatility_expanding", "low_volatility", "high_volatility"
        ]
    ...
    def get_feature_importance(self, model_name="rf", top_n=15):
        """
    ...
    def evaluate_model_performance(self, X_test=None, y_test=None):
        """
        Evaluate model performance on test data
        """
        try:
            if X_test is None or y_test is None:
                # Use a portion of historical data as test set
                X, y = self.prepare_training_data()
                if X is None:
                    return None
...
class MLEnhancedScoring:
    ...
    def __init__(self):
        ...
        self.ml_weight = 0.3  # 30% weight to ML, 70% to traditional analysis
        
    def enhanced_strategy_score(self, latest, previous, stock_data, nifty_data=None):
        """
        ...
        # Get ML prediction
        ml_probability = self.ml_predictor.predict_trade_success(stock_data, nifty_data)
        
        # Convert ML probability to score (0-10 scale)
        ml_score = ml_probability * 10
        
        # Combine scores
        combined_score = (
            traditional_score * (1 - self.ml_weight) + 
        ...
        if ml_probability > 0.7:
            ml_indicators.append("ml_high_confidence")
        elif ml_probability > 0.6:
            ml_indicators.append("ml_medium_confidence")
        elif ml_probability < 0.4:
            ml_indicators.append("ml_low_confidence")
        
        enhanced_indicators = matched_indicators + ml_indicators
        
        return combined_score, enhanced_indicators, {
    ...
    def should_trade_ml_filter(self, ml_probability, min_confidence=0.55):
        """
        ML-based filter for trade decisions
        """
        if ml_probability >= min_confidence:
            return True, f"ML confidence: {ml_probability:.2f}"
        else:
            return False, f"ML confidence too low: {ml_probability:.2f} < {min_confidence}"
...
if __name__ == "__main__":
    ...
    enhanced_scoring = MLEnhancedScoring()
    ...
    try:
        # Get test data
        stock_data = yf.download(test_ticker, period="6mo", interval="1d", progress=False)
        ...
        # Generate features
        features = feature_engineer.create_features(stock_data, nifty_data)
        
        print(f"✅ Generated {len(features)} features for {test_ticker}")
        ...
        probability = ml_predictor.predict_trade_success(stock_data, nifty_data)
        ...
        enhanced_score, indicators, details = enhanced_scoring.enhanced_strategy_score(
            latest, previous, stock_data, nifty_data
        )
        
        print(f"   Traditional Score: {details['traditional_score']:.2f}")
        print(f"   ML Score: {details['ml_score']:.2f}")
    ...
    X, y = ml_predictor.prepare_training_data(min_trades=10)  # Lower threshold for testing
    ...
    if X is not None:
        ...
        if success:
            ...
            results = ml_predictor.evaluate_model_performance()
            
            # Show feature importance
            print(f"\n🔍 Feature Importance (Random Forest):")
            top_features = ml_predictor.get_feature_importance("rf", top_n=10)
            for feature, importance in top_features:
                print(f"   {feature}: {importance:.3f}")
    else:
        print("⚠️ Insufficient training data for model training")
