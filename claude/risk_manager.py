...
if __name__ == "__main__":
    ...
    print(f"   Risk: {(entry_price - optimal_stop) / entry_price * 100:.1f}%")
    
    # Test trailing stop
    print(f"\n📈 Testing Trailing Stop...")
    current_price = 2600  # 4% profit
    current_stop = optimal_stop
    
    new_stop, trail_reason = stop_optimizer.update_trailing_stop(
        current_price, entry_price, current_stop
    )
    
    print(f"   Current Price: ₹{current_price}")
    print(f"   Old Stop: ₹{current_stop:.2f}")
    print(f"   New Stop: ₹{new_stop:.2f}")
    print(f"   Trail Reason: {trail_reason}")
    
    # Test portfolio monitoring
    print(f"\n📊 Testing Portfolio Monitoring...")
    portfolio_metrics = portfolio_monitor.calculate_portfolio_metrics()
    
    print(f"   Total Positions: {portfolio_metrics['total_positions']}")
    print(f"   Total Invested: ₹{portfolio_metrics['total_invested']:,.0f}")
    print(f"   Total Risk: ₹{portfolio_metrics['total_risk']:,.0f}")
    print(f"   Current P&L: ₹{portfolio_metrics['current_pnl']:,.0f}")
    
    # Check risk limits
    warnings = portfolio_monitor.check_risk_limits(1000000)
    if warnings:
        print(f"\n⚠️ Risk Warnings:")
